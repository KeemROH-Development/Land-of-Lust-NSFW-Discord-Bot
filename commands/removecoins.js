...
module.exports = {
    ...
    db.collection('Userinfo').doc(message.author.id).get().then((q)=>{
      if(!q.exists){
        return message.channel.send("You must be registered in the database to use this command.")//which is this?
      }
   if(q.exists){
   ...
   if(!blacklist) blacklist = false
    if(blacklist == true) return message.channel.send('You\'re blacklisted')
      ...
      db.collection('Userinfo').doc(member.user.id).get().then((b)=>{
        if(!b.exists){
          return message.channel.send('This user is not registered in our database')
        }
        if(b.exists){
          let coin;
    ...
    let setc = parseInt(arg)
        if(isNaN(arg)) return message.channel.send("Not a valid amount of coins") 
          db.collection('Userinfo').doc(member.user.id).update({ 
            'coins' :  coin -= setc
          }).then(()=>{
            message.channel.send("Coins successfully removed!")//i got it
          })
        })
        }
      })
